        ...
        <div class="cart-content">
            ...
            <div class="cart-list">
                ...
                <div class="cart-list row">
                    ...
                    <div class="col-xs-4 col-sm-4 no-padd-right"><span class="down2" onclick="javascript:down_cart('{{ $cartItem->rowId }}', {{ $cartItem->qty }});"  data-rowid="{{ $cartItem->rowId }}" data-qty="{{ $cartItem->qty }}">-</span> {{ $cartItem->qty }} <span class="up2" onclick="javascript:update_cart('{{ $cartItem->rowId }}', {{ $cartItem->qty }});" data-rowid="{{ $cartItem->rowId }}" data-qty="{{ $cartItem->qty }}">+</span></div>
                    <div class="col-xs-3 col-sm-3 no-padd-right price-color">{{ number_format($cartItem->price*$cartItem->qty, 0, ',', '.') }}<sup>đ</sup></div>
                    <div class="col-xs-2 col-sm-2 no-padd-right"><span class="cancel" onclick="javascript:del_item_cart('{{ $cartItem->rowId }}');" data-rowid="{{ $cartItem->rowId }}">x</span></div>
                </div>
                @endforeach
            </div>
            ...
            <div class="row">  
                <div class="col-xs-12 col-sm-12">
                    <select class="js-example-basic-single" name="client_ip_select" id="client_ip_select">
                        @foreach( $dataClient as $client )
                        <option value="{!! $client->client_ip !!}">{!! $client->client_name !!}</option>
                        @endforeach
                    </select>
                </div>                
            </div>            
            ...
            <div class="cart-button row">
                ...
                <div class="col-xs-6 col-sm-6 btn-ok">
                    <button type="button" class="pay" data-toggle="modal" data-target="#modal_order_details">Thanh toán</button>
                </div>
            </div>
        </div>
        </form>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="javascript:insert_order($('#order_notice').val(), $('#client_ip_select').val() );" >Đồng ý</button>
            </div>
        </div>
    </div>
</div>
