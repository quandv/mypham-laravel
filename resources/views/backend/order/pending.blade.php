@extends('backend.layouts.master',['page_title' => 'Đơn hàng đang xử lý'])
...
@section('after-scripts-end')
  <script type="text/javascript">
        ...
        function orderData(){
          ...
          $.get('{!!route('order.ajax.pending')!!}',{ 'pnum' : page_num, 'pnum2' : page_num2 },function(data){ 
        ...
        if (realtime) {
           clearInterval(getdata);
           timeing = 9000000000;
           //var time_rand = Math.floor((Math.random() * 1000) + 100);
            ...
            if (select_option != -1 && select_option != 4) {
                ...
                swal({
                    ...
                    $.ajax({
                      ...
                      type: 'POST',
                      data: { 'arr' : checkbox_arr,'status' : select_option,'arr_status' : status_arr,'arr_date' : date_arr,'arr_room' : room_arr,'chef_do' : chef_do},
                      dataType: 'html',
                    }).done(function(data){
                       $('select[name=select-option-chose] option[value=-1]').prop("selected", true);
                ...
                swal({
                  title: 'Lý do hủy đơn hàng !',
                  html: '<textarea class="form-control" rows="3" name="resonDestroy"></textarea><label class="radio-inline"><input type="radio" name="backStock" id="inlineRadio1" value="1" checked> Hoàn trả nguyên liệu</label><label class="radio-inline"><input type="radio" name="backStock" id="inlineRadio2" value="0" disabled> Không hoàn trả nguyên liệu</label>',
                  showCancelButton: true,
                  preConfirm: function() {
                    return new Promise(function(resolve, reject) {
                      if ( $('textarea[name=resonDestroy]').val() != '' ) {
