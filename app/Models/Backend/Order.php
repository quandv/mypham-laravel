        ...
        $data = DB::table("order")
           ...
           ->where(function ($query) use ($request){                
                ...
                if ($request->time_start != '') {
                    ...
                    $query->whereRaw("order.order_create_time >= ?", array($request->time_start));
                } 
                if ($request->time_end != '') {
                    $query->whereRaw("order.order_create_time <= ?", array($request->time_end));
                }   
         ...
         $data = DB::table("order")
           ...
           ->where(function($query) {
                 ...
                 if (access()->hasPermission('chef-do')) {
                    $query->where('order.order_chef_do', 1);
                 }
            })
           ->where(function ($query) use ($request){                
                if ($request->table_search != '' ) {
    ...
    if($request->page) $page = $request->page; else $page = 1;
       ...
       if(!empty($data)) { 
            foreach($data as $val) {
                  ...
                  $xhtml .= "<td>".$val->order_id."</td><td>".$val->client_name."</td>";
                  $xhtml .= "<td>".date("d-m-Y H:i:s",strtotime($val->order_create_time)) ."</td>";
                  $xhtml .=  '<td><div class="row"><div class="col-sm-4 no-padding">&nbsp;</div><div class="col-sm-2">Số lượng</div><div class="col-sm-3">Đơn giá</div><div class="col-sm-3">Thành tiền</div></div>';
                       for($i = 0 ; $i < count($val->product_name_group); $i++) {
                         $xhtml .='<div class="row"><div class="col-sm-4 no-padding">'. $val->product_name_group[$i];
                         if (!empty($val->product_option_group)) {
                          $xhtml .='<ul class="order-option">';
                        foreach ($val->product_option_group[$i] as $k => $v) {
                          $xhtml .= '<li>'.$v['1'].'('.number_format($v['2'],0,",",".").')('.$v['3'].')</li>';
                        ...
                        $xhtml .='</td>';
                  $xhtml .='<td class="text-right">'.number_format($val->sumPrice,0,",",".").'</td>';
                  $xhtml .='<td style="max-width:150px;min-width: 80px;">'.$val->order_notice.'</td>';
       ...
       if(!empty($data2)) { 
            foreach($data2 as $val2) {
                  ...
                  $xhtml2 .= "<td>".$val2->order_id."</td><td>".$val2->client_name."</td>";
                  $xhtml2 .= "<td>".date("d-m-Y H:i:s",strtotime($val2->order_create_time)) ."</td>";
                  $xhtml2 .=  '<td><div class="row"><div class="col-sm-4 no-padding">&nbsp;</div><div class="col-sm-2">Số lượng</div><div class="col-sm-3">Đơn giá</div><div class="col-sm-3">Thành tiền</div></div>';
                       for($i = 0 ; $i < count($val2->product_name_group); $i++) {
                         $xhtml2 .='<div class="row"><div class="col-sm-4 no-padding">'. $val2->product_name_group[$i];
                         if (!empty($val2->product_option_group)) {
                          $xhtml2 .='<ul class="order-option">';
                        foreach ($val2->product_option_group[$i] as $k => $v) {
                          $xhtml2 .= '<li>'.$v['1'].'('.number_format($v['2'],0,",",".").')('.$v['3'].')</li>';
                         ...
                         $xhtml2 .= '</div>';
                         $xhtml2 .= '<div class="col-sm-2">'. $val2->product_qty_group[$i] .'</div>';
                         $xhtml2 .= '<div class="col-sm-3">'. number_format($val3->product_price_group[$i],0,",",".") .'</div>';
                         $xhtml2 .= '<div class="col-sm-3">'. number_format($val2->price[$i],0,",",".") .'</div></div>';
                       }
                        $xhtml2 .='</td>';
                  $xhtml2 .='<td class="text-right">'.number_format($val2->sumPrice,0,",",".").'</td>';
                  $xhtml2 .='<td style="max-width:150px;min-width: 80px;">'.$val2->order_notice.'</td>';
